...
impl Game {
    ...
    fn play(&self) -> i32 {
        match *self {
            Game { opponent: Shape::Rock, player: Shape::Paper } => 6,
            Game { opponent: Shape::Rock, player: Shape::Scissor } => 0,
            Game { opponent: Shape::Paper, player: Shape::Rock } => 0,
            Game { opponent: Shape::Paper, player: Shape::Scissor } => 6,
            Game { opponent: Shape::Scissor, player: Shape::Rock } => 6,
            Game { opponent: Shape::Scissor, player: Shape::Paper } => 0,
            _ => 3
        }
    }
}
...
fn main() {
    println!("the score is {}", solve_first_problem("./data/day2/input.txt"));
    println!("The score with the secret is {}", solve_second_problem("./data/day2/input.txt"));
}
